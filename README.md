...
## 프로젝트 소개
이 프로젝트는 사용자가 여러 할 일을 생성하고 관리할 수 있는 웹 애플리케이션입니다. 프로젝트를 추가할 때 제목, 내용, 마감일을 설정할 수 있으며, 추가된 할 일은 좌측 메뉴에 리스트로 나타납니다. 할 일 리스트 중 하나를 클릭하면 해당 할 일의 상세 화면으로 이동하며, 기본적으로 작성한 제목, 내용, 마감일이 표시됩니다.

상세 화면에서는 개별 업무를 추가 및 삭제할 수 있고, 할 일 자체도 삭제할 수 있는 기능이 구현되어 있습니다. 또한, 애플리케이션 로드 시 사용자 이름을 입력하는 화면이 나타나며, 할 일 데이터와 업무 데이터가 LocalStorage에 저장되어 데이터가 지속됩니다.

## 프로젝트 링크
- [할 일 관리 앱 링크](https://gkkary3.github.io/react-To-do-List/)
...
1. **할 일 추가**  
   - 제목, 내용, 마감일을 설정하여 할 일을 추가할 수 있습니다.
   - 추가된을 할 일들은 좌측 메뉴에 리스트 형태로 표시됩니다.

2. **할 일 상세 화면**  
   - 리스트에서 할 일을 클릭하면 상세 화면으로 이동합니다.
   - 상세 화면에서 할 일에 업무를 추가하거나 삭제할 수 있습니다.
   - 할 일 자체를 삭제하는 기능도 제공됩니다.

3. **React Hooks 활용**  
   - `useState`: 할 일과과 업무 상태 관리
   - `useRef`: 폼 입력 처리
...
5. **사용자 이름 입력 및 LocalStorage 저장**  
   ...
   - 할 일 데이터와 업무 데이터 또한 LocalStorage에 저장되어 브라우저를 재로드해도 데이터가 유지됩니다.
